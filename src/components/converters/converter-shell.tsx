...
import { loadConverter } from "@/lib/converters/loaders";
import { formatConversionValue } from "@/lib/format";
import { useConversionHistory } from "@/hooks/use-conversion-history";
import { useRecent } from "@/hooks/use-recent";
import { SITE_URL } from "@/lib/constants";
import type { NumericConverterDefinition, UnitDefinition } from "@/lib/converters/types";
...
export function ConverterShell({ slug, units, defaultFromUnit, defaultToUnit }: ConverterShellProps) {
  const [convert, setConvert] = useState<NumericConverterDefinition["convert"] | null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    loadConverter(slug).then((def) => {
      // ConverterShell only renders for numeric converters (see converters/[slug]/page.tsx).
      if (!cancelled && def.kind === "numeric") setConvert(() => def.convert);
    });
  ...
  const result = useMemo(() => {
    if (!convert || inputValue === "" || Number.isNaN(numericInput)) return null;
    try {
      return convert(numericInput, fromUnit, toUnit);
    } catch {
      return null;
    }
  }, [convert, numericInput, inputValue, fromUnit, toUnit]);
    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-border bg-card p-6 shadow-soft sm:p-8">
        ...
        <div className="mt-5 flex flex-wrap gap-2" data-no-print="true">
          ...
          <ShareButton title="Conversion result" url={shareUrl} />
          <PrintButton />
